...
ADP 시험 대비이므로 데이콘 데이터의 일부를 이용해서 튜토리얼을 작성해보겠다.
...
# sampling 
set.seed(123)
ind <- sample(1:26457, 1000, replace = F)
train <- train[ind, ]

ind_test <- sample(1:10000, 300, replace = F)
test <- test[ind_test, ]
```
...
# 51	28	15	0.002188723	1.791991e-06	0.8873527	Preprocessor1_Model10
```
...
final_xgb %>% 
    ...
    pull_workflow_fit() %>% 
    vip(geom = 'point') 
# begin_month, month_birth, month_employed, begin_year, income_total, year_birth, year_employed, car, phone, occp_type
```
